...
tuple<uint8_t*, uint8_t*, uint8_t*> encoder::encode(string data, size_t* len)
{
    ...
    // thread 1 starts at last even index
    ssize_t start1 = (n - 1) - ((n - 1) % 2 == 0 ? 0 : 1);

    // Launch threads
    thread t0(&encoder::encode_half, this, cref(data), start0, 2, ref(state0), &ptr0);

    thread t1(&encoder::encode_half, this, cref(data), start1, 2, ref(state1), &ptr1);
...
string decoder::decode(uint8_t** ptr0, uint8_t** ptr1) {
    ...
    *ptr1 = pptr1;
    ...
    string decoded_data0 = "";
    string decoded_data1 = "";
    
    uint32_t num_symbols_0 = (data_size / 2);
    uint32_t num_symbols_1 = (data_size - num_symbols_0);  // remaining go to stream1
    
    thread t0(&decoder::decode_half, this, num_symbols_0, ptr0, state0, ref(decoded_data0), false);
    thread t1(&decoder::decode_half, this, num_symbols_1, ptr1, state1, ref(decoded_data1), true);
    ...
    for (uint32_t i = 0; i < num_symbols_0; i++) {
        decoded_data += decoded_data1[i];
        decoded_data += decoded_data0[i];
    }
    if (data_size & 1u) {
        decoded_data += decoded_data1[num_symbols_0];
    }
