...
void encoder::encode_symbol(char s, uint8_t** ptr) {
...
tuple<uint8_t*, uint8_t*> encoder::encode(string data, size_t* len)
{
    ...
    state0 = params.INITIAL_STATE;
    state1 = params.INITIAL_STATE;

    for (size_t i = n; i > 0; i--) {
        char s = data[i-1];
        encode_symbol(s, &ptr0);
    }

    // Store final states
    uint32_t state_first = (n & 1u) ? state1 : state0;
    uint32_t state_second = (n & 1u) ? state0 : state1;
    
    ptr0 -= 4;
    ptr0[0] = (state_first >> 0);
    ptr0[1] = (state_first >> 8);
    ptr0[2] = (state_first >> 16);
    ptr0[3] = (state_first >> 24);

    ptr0 -= 4;
    ptr0[0] = (state_second >> 0);
    ptr0[1] = (state_second >> 8);
    ptr0[2] = (state_second >> 16);
    ptr0[3] = (state_second >> 24);
...
char decoder::decode_symbol(uint8_t** ptr) {
    uint32_t state = state0;
...
string decoder::decode(uint8_t** ptr0) {
    uint8_t* pptr0 = *ptr0;
    ...
    data_size = pptr0[0] << 0;
    data_size |= pptr0[1] << 8;
    data_size |= pptr0[2] << 16;
    data_size |= pptr0[3] << 24;
    pptr0 += 4;

    // get final states from bitarray
    uint32_t state_first = 0;
    uint32_t state_second = 0;
    state_second = pptr0[0] << 0;
    state_second |= pptr0[1] << 8;
    state_second |= pptr0[2] << 16;
    state_second |= pptr0[3] << 24;
    pptr0 += 4;
    
    state_first = pptr0[0] << 0;
    state_first |= pptr0[1] << 8;
    state_first |= pptr0[2] << 16;
    state_first |= pptr0[3] << 24;
    pptr0 += 4;
    *ptr0 = pptr0;
...
bool test_rANS(uint32_t& enc_avg_time, uint32_t& dec_avg_time) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        string decoded_data = dec.decode(&ptr0_begin);
...
void write_bitarray_to_file(uint8_t* ptr0, const size_t len, const string& file_path) {
...
tuple<int, int, string> run_test(string data, string input_file_path, string output_file_path, rANSParams params, bool verbose) {
    ...
    auto dec_start = chrono::high_resolution_clock::now();
    string decoded_data = dec.decode(&encoded_begin_ptr0);
    auto dec_stop = chrono::high_resolution_clock::now();
